package com.crawljax.hoverdetection;
 ...
 * <p>
 * For example usage, see HoverDetectionExample in examples module
 */
public class HoverDetectorPlugin implements OnNewStatePlugin {

    private static final Logger LOG = LoggerFactory.getLogger(HoverDetectorPlugin.class);
    public static final String CDP_SCRIPT = "function getEventHandlers(xpath){\n" + "	// a= $x(xpath)[0];\n"
            ...
            + "		return returnMap;\n"
            + "	}\n"
            + "	if(getEventListeners(a)['mouseover']){\n"
            + "		returnMap['eventListeners'] =  getEventListeners(a)['mouseover'][0].listener.toString()\n"
            + "	} \n"
            + "\n"
            + "	return returnMap;\n"
            + "}";
    ...
    public void onNewState(CrawlerContext context, StateVertex newState) {
        ...
        if (isChrome) {
            try {
                findHovers(context.getBrowser(), newState);
            } catch (IOException e) {
                LOG.error("Error extracting hovers from {}", newState.getName());
                LOG.debug(e.getMessage());
            }
        } else {
            throw new IllegalStateException("Cannot call HoverDetector if the browser is not CHROME with CDP.\n"
                    + "Make sure BrowserType is CHROME and Please use BrowserOptions.setUSE_CDP(true) to enable CDP");
        }
    }

    public void findHovers(EmbeddedBrowser browser, StateVertex newState) throws IOException {
        Document dom = newState.getDocument();
        if (isPopulated(dom)) {
            LOG.info("Already populated dom. No need to run hover detection again");
            return;
        }
