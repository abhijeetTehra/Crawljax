...
public final class WebDriverBackedEmbeddedBrowser implements EmbeddedBrowser {
     ...
     */
    private WebDriverBackedEmbeddedBrowser(
            WebDriver driver, ImmutableSortedSet<String> filterAttributes, long crawlWaitReload, long crawlWaitEvent) {
        this.browser = driver;
     ...
     * @return The EmbeddedBrowser.
     */
    public static WebDriverBackedEmbeddedBrowser withDriver(
            WebDriver driver, ImmutableSortedSet<String> filterAttributes, long crawlWaitEvent, long crawlWaitReload) {
    ...
    private static RemoteWebDriver buildRemoteWebDriver(String hubUrl, DesiredCapabilities desiredCapabilities) {
        ...
        try {
            ...
            LOGGER.error("The given hub url of the remote server is malformed can not continue!", e);
            return null;
        }
    ...
    private void locateFrameAndGetSource(Document document, String topFrame, Element frameElement)
        ...
        if (nameId != null && !ignoreFrameChecker.isFrameIgnored(frameIdentification + nameId)) {
            ...
            try {
                ...
                LOGGER.error("Cannot switch to frame {}", frameIdentification);
                LOGGER.debug(e.getMessage(), e);
                browser.switchTo().defaultContent();
                return;
            }
            ...
            try {
                ...
                LOGGER.info("Got exception while inspecting a frame: {} continuing...", frameIdentification, e);
            }
        }
    }
        ...
        switch (input.getType()) {
            ...
            case TEXTAREA:
                values.add(new InputValue(
                        new RandomInputValueGenerator().getRandomString(FormHandler.RANDOM_STRING_LENGTH), true));
                break;
            case CHECKBOX:
            case RADIO:
            ...
            case NUMBER:
                ...
                values.add(new InputValue(new RandomInputValueGenerator().getRandomNumber(), true));
                break;
            case SELECT:
                Select select = new Select(webElement);
                if (!select.getOptions().isEmpty()) {
                    WebElement option = new RandomInputValueGenerator().getRandomItem(select.getOptions());
    ...
    public BufferedImage getScreenShotAsBufferedImage(int scrollTime) {
        ...
        try {
            final ShootingStrategy pasting =
                    new ViewportPastingDecorator(new SimpleShootingStrategy()).withScrollTimeout(scrollTime);
    ...
    private void removeCanvasGeneratedByFirefoxDriverForScreenshots() {
        ...
        try {
            ...
            LOGGER.error(
                    "Removing of Canvas Generated By FirefoxDriver failed," + " most likely leaving it in the browser",
                    e);
        }
    }
